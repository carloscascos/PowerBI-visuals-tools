...
export class Visual implements IVisual {
    ...
    constructor(options: VisualConstructorOptions) {
        ...
        this.pathLayer = this.mainGroup.append('g').classed('pathLayer', true);
        this.glyphLayer = this.mainGroup.append('g').classed('glyphLayer', true);
        ...
        this.zoomBehavior = d3.zoom<SVGElement, unknown>()
            ...
            .on('zoom', (event) => {
                ...
                this.svg.style('cursor', event.sourceEvent ? 'grabbing' : 'grab');
            })
            .on('end', () => {
    ...
    private parseData(dataView?: DataView): PathGroup[] {
        ...
        if (latIndex === -1 || lonIndex === -1 || !categorical.values) return [];

        const rowCount = categorical.values[0].values.length;
        
        for (let i = 0; i < rowCount; i++) {
            const pathId = pathIdIndex >= 0 && categorical.categories ? 
                String(categorical.categories[pathIdIndex].values[i]) : 'default';
            const timestamp = timestampIndex >= 0 && categorical.categories ? 
                new Date(categorical.categories[timestampIndex].values[i] as any) : new Date(i * 1000);
    ...
    private renderGlyphs(pathGroup: PathGroup, projection: (coords: [number, number]) => [number, number]) {
        ...
        this.glyphLayer.append('circle')
            .attr('cx', startX)
            .attr('cy', startY)
            .attr('r', glyphSettings.glyphSize.value / 4)
        ...
        this.glyphLayer.append('path')
            ...
            .attr('transform', `translate(${endX},${endY}) rotate(${angle}) scale(${glyphSettings.glyphSize.value / 20})`)
    ...
    private renderDirectionalArrows(
        ...
        for (let i = 1; i <= arrowCount; i++) {
            ...
            this.glyphLayer.append('path')
                ...
                .attr('transform', `translate(${point.x},${point.y}) rotate(${angle}) scale(${glyphSettings.glyphSize.value / 10})`)
